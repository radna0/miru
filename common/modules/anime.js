...
export async function getChaptersAniSkip(file, duration) {
  const resAccurate = await fetch(
    `https://api.aniskip.com/v2/skip-times/${file.media.media.idMal}/${file.media.episode}/?episodeLength=${duration}&types=op&types=ed&types=recap`
  );
  const jsonAccurate = await resAccurate.json();

  const resRough = await fetch(
    `https://api.aniskip.com/v2/skip-times/${file.media.media.idMal}/${file.media.episode}/?episodeLength=0&types=op&types=ed&types=recap`
  );
...
export function getMediaMaxEp(media, playable) {
  if (playable) {
    return (
      ...
      media.airingSchedule?.nodes?.[0]?.episode - 1 ||
      media.episodes
    );
  } else {
    return (
